...
`tier-a-events.jsonl`, `tier-b-events.jsonl`, `human-required-events.jsonl`, `ambient-behavior-events.jsonl`, `qualified-handoff-events.jsonl`, `qualified-handoff-no-target-events.jsonl`, and `derived-events.jsonl` must pass. `fake-collaboration-events.jsonl`, `ambient-leakage-events.jsonl`, `invalid-evidence-events.jsonl`, `invalid-derived-events.jsonl`, and `task-binding-adversarial-events.jsonl` must fail because their intentionally attempted projections are rejected. A failing fixture is an expected negative test, not a validator malfunction.
